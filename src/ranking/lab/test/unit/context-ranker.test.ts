...
Deno.test("Context Ranker - Day of Week Adjustments", () => {
  ...
  const mondayLifestyleScore = applyContextualAdjustments(5, mondayContext, lifestyleArticle);
  const sundayLifestyleScore = applyContextualAdjustments(5, sundayContext, lifestyleArticle);
  
  console.log(`Monday tech: ${mondayTechScore}, Sunday tech: ${sundayTechScore}`);
  console.log(`Monday lifestyle: ${mondayLifestyleScore}, Sunday lifestyle: ${sundayLifestyleScore}`);
});
...
Deno.test("Context Ranker - Time of Day Adjustments", () => {
  ...
  const morningNewsScore = applyContextualAdjustments(5, morningContext, newsArticle);
  const nightNewsScore = applyContextualAdjustments(5, nightContext, newsArticle);
  
  const morningEntertainmentScore = applyContextualAdjustments(5, morningContext, entertainmentArticle);
  const nightEntertainmentScore = applyContextualAdjustments(5, nightContext, entertainmentArticle);
  
  console.log(`Morning news: ${morningNewsScore}, Night news: ${nightNewsScore}`);
  console.log(`Morning entertainment: ${morningEntertainmentScore}, Night entertainment: ${nightEntertainmentScore}`);
});
...
Deno.test("Context Ranker - Mood-Based Adjustments", () => {
  ...
  const entertainmentContext = { ...mockContexts[0], userMood: 'entertainment' as const };
  
  const focusedTechScore = applyContextualAdjustments(5, focusedContext, techArticle);
  const learningEducationalScore = applyContextualAdjustments(5, learningContext, educationalArticle);
  const entertainmentFunScore = applyContextualAdjustments(5, entertainmentContext, entertainmentArticle);
...
Deno.test("Context Ranker - Score Bounds", () => {
  ...
  assertEquals(overHighScore <= 10, true, "Adjusted score should be capped at 10");
  assertEquals(midScore >= 0 && midScore <= 10, true, "Mid-range scores should stay in bounds");
});
...
Deno.test("Context Ranker - Contextual Adjustments Calculation", () => {
  ...
  const adjustments = calculateContextualAdjustments(mondayMorningContext, techArticle);
...
Deno.test("Context Ranker - Different Contexts Produce Different Results", () => {
  ...
  const mondayScore = applyContextualAdjustments(baseScore, mockContexts[0], article);
  const sundayScore = applyContextualAdjustments(baseScore, mockContexts[1], article);
  const fridayScore = applyContextualAdjustments(baseScore, mockContexts[2], article);
  
  const scores = [mondayScore, sundayScore, fridayScore];
  const uniqueScores = [...new Set(scores)];
  
  console.log(`Monday: ${mondayScore}, Sunday: ${sundayScore}, Friday: ${fridayScore}`);
