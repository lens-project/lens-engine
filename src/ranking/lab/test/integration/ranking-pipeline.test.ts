...
Deno.test("Integration - ContentRanker End-to-End Pipeline", async () => {
  ...
  assert(
    scoringResult.score >= 0 && scoringResult.score <= 10,
    "Score in valid range",
  );
  assert(
    scoringResult.confidence >= 0 && scoringResult.confidence <= 1,
  ...
  console.log(
    `Pipeline result: Score ${scoringResult.score}, Method: ${scoringResult.method}`,
  );
...
Deno.test("Integration - Context-Aware Scoring Differences", async () => {
  ...
  const mondayMorningScore = await ranker.scoreArticle(
    article,
    mockContexts[0],
  ); // Monday morning, focused
  const sundayAfternoonScore = await ranker.scoreArticle(
    article,
    mockContexts[1],
  ); // Sunday afternoon, casual
  const fridayEveningScore = await ranker.scoreArticle(
    article,
    mockContexts[2],
  ...
  assert(
    !("type" in sundayAfternoonScore),
    "Sunday result should not be error",
  );
  ...
  console.log(
    `Context-aware scores - Monday: ${mondayScore}, Sunday: ${sundayScore}, Friday: ${fridayScore}`,
  );
...
Deno.test("Integration - Relevance Categorization", async () => {
  ...
  assert(
    !("type" in mediumResult),
    "Medium quality result should not be error",
  );
  ...
  console.log(
    `Categorization - High: ${highScore}→${highCategory}, Low: ${lowScore}→${lowCategory}, Medium: ${mediumScore}→${mediumCategory}`,
  );

  // Verify scoring trends match expected patterns
  assert(
    highScore > lowScore,
    "High quality should score higher than clickbait",
  );
  assert(
    lowCategory === "skip" || lowCategory === "maybe-interesting",
    "Clickbait should be skip or maybe",
  );
});
...
Deno.test("Integration - Functional API Methods", async () => {
  ...
  assertEquals(
    category,
    "high-interest",
    "Functional categorization should work",
  );

  console.log(
    `Functional API - Single: ${
      (singleResult as ScoringResult).score
    }, Batch: ${batchResults[0].score}, Category: ${category}`,
  );
});
...
Deno.test("Integration - Performance Characteristics", async () => {
  ...
  console.log(
    `Performance - Single: ${singleDuration.toFixed(2)}ms, Batch (3): ${
      batchDuration.toFixed(2)
    }ms`,
  );
  ...
  assert(
    singleDuration < 1000,
    "Single article should complete in under 1 second",
  );
...
Deno.test("Integration - Score Consistency and Determinism", async () => {
  ...
  console.log(
    `Consistency test - ${results.length} runs, all scored: ${results[0]}`,
  );
});
...
Deno.test("Integration - Full Article Dataset Processing", async () => {
  ...
  const mediumScores =
    results.filter((r) => r.score >= 4 && r.score < 7).length;
  const lowScores = results.filter((r) => r.score < 4).length;

  console.log(
    `Full dataset processing: ${results.length} articles in ${
      duration.toFixed(2)
    }ms`,
  );
  console.log(
    `Score distribution - High: ${highScores}, Medium: ${mediumScores}, Low: ${lowScores}`,
  );
...
Deno.test("Integration - Context Factor Tracking", async () => {
  ...
  if (scoringResult.contextFactors) {
    ...
    assertEquals(
      typeof scoringResult.contextFactors.dayOfWeekAdjustment,
      "number",
    );

    console.log(
      `Context factors - Day adjustment: ${scoringResult.contextFactors.dayOfWeekAdjustment}`,
    );
  }
});
