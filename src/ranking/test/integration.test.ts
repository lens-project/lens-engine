...
import { isRankingError, isScoringResult, formatRankingResults } from '../src/utils.ts';
...
Deno.test("Integration - Public API Functional Interface", async () => {
  ...
  if (isScoringResult(singleResult)) {
    ...
    assert(singleResult.score >= 0 && singleResult.score <= 10, "Score in valid range");
    assertEquals(singleResult.method, 'llm', "Should use LLM method");
  } else {
    console.log(`⚠️  Single article ranking failed: ${singleResult.type} - ${singleResult.message}`);
  }
...
Deno.test("Integration - Class-Based API", async () => {
  ...
  if (isScoringResult(result)) {
    ...
    assert(['high-interest', 'maybe-interesting', 'skip'].includes(category), 
           "Should return valid category");
  } else {
    console.log(`⚠️  Class API ranking failed: ${result.type} - ${result.message}`);
  }
});
...
Deno.test("Integration - Context Creation Helper", () => {
  ...
  assertEquals(context.readingDuration, 'medium', "Should have specified duration");
  ...
  console.log(`Current context: ${context.dayOfWeek} ${context.timeOfDay}, ${context.userMood}, ${context.readingDuration}`);
});
...
Deno.test("Integration - Relevance Categorization", () => {
  ...
  for (const score of testScores) {
    ...
    if (score >= 7) {
      assertEquals(category, 'high-interest', `Score ${score} should be high-interest`);
    } else if (score >= 4) {
      assertEquals(category, 'maybe-interesting', `Score ${score} should be maybe-interesting`);
    } else {
...
Deno.test("Integration - Batch Processing with Mixed Results", async () => {
  ...
  assertEquals(results.length, mixedArticles.length, "Should return result for each article");
  
  const successful = results.filter(isScoringResult);
  const errors = results.filter(isRankingError);
  
  console.log(`Mixed batch results: ${successful.length} successful, ${errors.length} errors`);
...
Deno.test("Integration - Performance and Statistics", async () => {
  ...
  console.log(`Batch ranking performance: ${duration.toFixed(2)}ms for ${testArticles.length} articles`);
  ...
  assert(formattedResults.includes('Ranking Results Summary'), "Should include summary header");
...
Deno.test("Integration - Error Resilience", async () => {
  ...
  const results = await ranker.rankBatch(testArticles.slice(0, 2), testContexts[0]);
  ...
  for (const result of results) {
    if (isRankingError(result)) {
      assert(['llm_error', 'context_error', 'timeout', 'invalid_input'].includes(result.type),
  ...
  console.log(`Error resilience test: ${results.filter(isScoringResult).length} successes, ${results.filter(isRankingError).length} errors`);
});
...
Deno.test({
  ...
  fn: async () => {
    ...
    if (avgDuration < 60000) { // Less than 1 minute average
      ...
      console.log("⚠️  Performance is slower than expected (may be normal for first runs)");
    }
  }
});