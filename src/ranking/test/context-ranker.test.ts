...
Deno.test("Production Context Ranker - Day of Week Adjustments", () => {
  ...
  const _financeArticle = testArticles[1]; // Market analysis
  ...
  assert(
    mondayTechScore >= sundayTechScore,
    "Monday should favor technical content",
  );
});
...
Deno.test("Production Context Ranker - Time of Day Adjustments", () => {
  ...
  const morningScore = applyContextualAdjustments(
    5,
    morningContext,
    productivityArticle,
  );
  const nightScore = applyContextualAdjustments(
    5,
    nightContext,
    productivityArticle,
  );
  ...
  console.log(
    `Morning productivity: ${morningScore}, Night productivity: ${nightScore}`,
  );

  // Morning should favor actionable/productivity content
  assert(
    morningScore >= nightScore,
    "Morning should favor productivity content",
  );
});
...
Deno.test("Production Context Ranker - Mood-Based Adjustments", () => {
  ...
  const focusedScore = applyContextualAdjustments(
    5,
    focusedContext,
    techArticle,
  );
  const casualScore = applyContextualAdjustments(5, casualContext, techArticle);
  const learningScore = applyContextualAdjustments(
    5,
    learningContext,
    techArticle,
  );
  ...
  console.log(
    `Focused: ${focusedScore}, Casual: ${casualScore}, Learning: ${learningScore}`,
  );

  // Learning and focused moods should favor technical content
  assert(
    learningScore >= casualScore,
    "Learning mood should favor tech content",
  );
...
Deno.test("Production Context Ranker - Score Bounds", () => {
  ...
  for (const inputScore of scores) {
    ...
    assert(
      adjustedScore >= 0,
      `Score ${adjustedScore} should be >= 0 (input: ${inputScore})`,
    );
    assert(
      adjustedScore <= 10,
      `Score ${adjustedScore} should be <= 10 (input: ${inputScore})`,
    );
  }
});
...
Deno.test("Production Context Ranker - Contextual Adjustments Calculation", () => {
  ...
  assert(
    adjustments.dayOfWeekMultiplier >= 0.5 &&
      adjustments.dayOfWeekMultiplier <= 1.5,
  );
  assert(
    adjustments.timeOfDayMultiplier >= 0.5 &&
      adjustments.timeOfDayMultiplier <= 1.5,
  );
  assert(
    adjustments.moodMultiplier >= 0.5 && adjustments.moodMultiplier <= 1.5,
  );
});
...
Deno.test("Production Context Ranker - Different Articles Respond Differently", () => {
  ...
  const techScore = applyContextualAdjustments(
    baseScore,
    context,
    testArticles[0],
  );
  const financeScore = applyContextualAdjustments(
    baseScore,
    context,
    testArticles[1],
  );
  const productivityScore = applyContextualAdjustments(
    baseScore,
    context,
    testArticles[2],
  );
  ...
  console.log(
    `Tech: ${techScore}, Finance: ${financeScore}, Productivity: ${productivityScore}`,
  );
  ...
  assert(
    ...
    "Different article types should produce different adjustments",
  );
});
