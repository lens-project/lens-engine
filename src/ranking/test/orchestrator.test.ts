...
import {
  ...
  testArticles,
  testContexts,
} from "./fixtures/test-articles.ts";
import {
...
Deno.test("Production Orchestrator - Configuration Integration", async () => {
  ...
  assertExists(
    ...
    "Ollama base URL should be configured",
  );
...
Deno.test("Production Orchestrator - Validation Methods", () => {
  ...
  // Test valid inputs
  assert(
    ranker.validateArticle(testArticles[0]),
    "Valid article should pass validation",
  );
  assert(
    ranker.validateContext(testContexts[0]),
    "Valid context should pass validation",
  );

  // Test invalid inputs
  assert(
    !ranker.validateArticle(invalidArticle as Partial<typeof testArticles[0]>),
    "Invalid article should fail validation",
  );
  assert(
    !ranker.validateContext(invalidContext as Partial<typeof testContexts[0]>),
...
Deno.test("Production Orchestrator - Input Validation Errors", async () => {
  ...
  const result = await ranker.rankArticle(
    invalidArticle as Partial<typeof testArticles[0]>,
    testContexts[0],
  );
...
Deno.test("Production Orchestrator - Context Adjustments", async () => {
  ...
  if (isScoringResult(mondayResult) && isScoringResult(sundayResult)) {
    console.log(
      `Monday score: ${mondayResult.score}, Sunday score: ${sundayResult.score}`,
    );
    ...
    console.log(
      `Context adjustment: ${
        mondayResult.contextFactors?.dayOfWeekAdjustment || 0
      }`,
    );
  } else {
    console.log(
      "One or both rankings failed - this is expected if Ollama is not available",
    );
  }
});
...
Deno.test("Production Orchestrator - Error Handling", async () => {
  ...
  if (isRankingError(result)) {
    assert(
      ["llm_error", "context_error", "timeout"].includes(result.type),
      "Should be a recognized error type",
    );
    assertExists(result.message, "Should have error message");
    console.log(
      `Expected error with short timeout: ${result.type} - ${result.message}`,
    );
  } else {
    console.log(
      "Ranking succeeded despite short timeout - system is very fast!",
    );
  }
});
...
Deno.test("Production Orchestrator - Configuration Options", () => {
  ...
  assert(
    ranker.validateArticle(testArticles[0]),
    "Validation should work with custom options",
  );
});
  ...
  fn: async () => {
    ...
    try {
      ...
      if (isScoringResult(result)) {
        ...
        console.log(
          `❌ Real LLM ranking failed: ${result.type} - ${result.message}`,
        );

        // This is acceptable if Ollama is not running
        assert(
          ["llm_error", "context_error"].includes(result.type),
