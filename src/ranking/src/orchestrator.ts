...
export class ContentRanker {
  ...
  async rankArticle(
    ...
    try {
      ...
      if (
        baseScoringResult.confidence <
          (mergedOptions.confidenceThreshold || 0.5)
      ) {
        console.warn(
          `Low confidence score (${baseScoringResult.confidence}) for article: ${article.title}`,
        );
      }
  ...
  async rankBatch(
    ...
    if (!this.cachedCriteria && !mergedOptions.criteriaConfig) {
      ...
      console.log(
        `✅ Loaded criteria: "${
          this.cachedCriteria.description || "Default criteria"
        }" (v${this.cachedCriteria.version})`,
      );
      console.log(
        `   📊 ${this.cachedCriteria.criteria.length} criteria, ${this.cachedCriteria.scoringGuidelines.length} scoring levels`,
      );
    }

    // Use cached criteria or provided criteria
    mergedOptions.criteriaConfig = mergedOptions.criteriaConfig ||
      this.cachedCriteria;
    ...
    console.log(
      `🔄 Processing ${articles.length} articles in batches of ${batchSize}...`,
    );
    ...
    for (let i = 0; i < articles.length; i += batchSize) {
      ...
      console.log(
        `📦 Batch ${batchNumber}/${totalBatches}: Processing ${batch.length} articles (${
          i + 1
        }-${i + batch.length})...`,
      );
      ...
      const batchPromises = batch.map((article, index) => {
        console.log(
          `   🔍 [${i + index + 1}/${articles.length}] Ranking: ${
      ...
      try {
        ...
        const successful = batchResults.filter((r) =>
          !this.isRankingErrorData(r)
        ).length;
        const errors = batchResults.filter((r) =>
          this.isRankingErrorData(r)
        ).length;
        console.log(
          `   ✅ Batch ${batchNumber} complete: ${successful} successful, ${errors} errors`,
        );
      } catch (error) {
        console.log(
        ...
        if (mergedOptions.continueOnError) {
          ...
          for (const article of batch) {
            try {
              console.log(
                `   🔄 Individual retry: ${article.title.substring(0, 50)}${
                  article.title.length > 50 ? "..." : ""
                }`,
              );
              ...
              const errorMessage = individualError instanceof Error
                ...
                : "Unknown error";
              console.log(
                `   ❌ Individual failed: ${errorMessage.substring(0, 100)}${
  ...
  private validateInputs(article: ArticleInput, context: RankingContext): void {
    if (!this.validateArticle(article)) {
      throw {
        type: "invalid_input",
        message:
          "Invalid article input: title, summary, and valid URL are required",
        input: article,
        context,
      } as RankingErrorData;
    }
    ...
    if (!this.validateContext(context)) {
      throw {
        type: "invalid_input",
        message:
          "Invalid ranking context: invalid day, time, mood, or duration",
        input: article,
        context,
      } as RankingErrorData;
    }
  ...
  private isRankingErrorData(obj: unknown): obj is RankingErrorData {
