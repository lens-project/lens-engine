...
const DEFAULT_CRITERIA_CONFIG: RankingCriteriaConfig = {
  ...
  criteria: [
    ...
    },
    {
      ...
      description: "How well does this match the current context (day/time/mood)?",
      weight: 7
    },
    {
      id: "practical_value",
      name: "Practical Value",
      description: "Does this provide actionable insights or useful information?",
      weight: 6
    },
    {
      id: "uniqueness",
      name: "Uniqueness",
      description: "Is this offering new perspectives or just repeating common knowledge?",
      weight: 5
    },
    {
  ...
  scoringGuidelines: [
    {
      range: "0-2",
      description: "Poor quality, clickbait, completely irrelevant, or misleading content",
      examples: ["Obvious clickbait", "Factually incorrect", "Spam or promotional"]
    },
    {
      range: "3-4", 
      description: "Low quality but might have minimal interest, repetitive or shallow content",
      examples: ["Rehashed content", "Minimal new insights", "Poor writing quality"]
    },
    {
      range: "5-6",
      description: "Average quality, moderately interesting, standard coverage of topic",
      examples: ["Decent article", "Some useful information", "Standard treatment"]
    },
    {
      range: "7-8",
      description: "High quality, very relevant, valuable insights, well-researched content",
      examples: ["Well-researched", "Unique insights", "Highly relevant"]
    },
    {
      range: "9-10",
      description: "Exceptional quality, must-read content, unique insights, perfect context match",
      examples: ["Groundbreaking insights", "Perfect timing", "Life-changing content"]
    }
  ],
 ...
 */
export async function loadRankingCriteria(verbose = false): Promise<RankingCriteriaConfig> {
  try {
    ...
    try {
      ...
      if (verbose) {
        console.log(`✅ Loaded custom criteria: "${criteria.description || 'Custom criteria'}" (v${criteria.version})`);
        console.log(`   📊 ${criteria.criteria.length} criteria, ${criteria.scoringGuidelines.length} scoring levels`);
      }
      ...
      if (error instanceof Deno.errors.NotFound) {
        if (verbose) {
          ...
          console.log(`   💡 Create ${criteriaPath} to customize ranking criteria`);
        }
        return DEFAULT_CRITERIA_CONFIG;
      } else {
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.warn(`⚠️  Failed to load criteria from ${criteriaPath}: ${errorMessage}`);
 ...
 */
export async function createExampleCriteriaConfig(dataDir: string): Promise<void> {
  ...
  const exampleConfig = {
    ...
    description: "Example custom ranking criteria - modify to suit your preferences",
    "_comments": {
      "version": "Configuration version for future compatibility",
      "criteria": "List of evaluation criteria - modify weights to prioritize different aspects",
      "scoringGuidelines": "Score ranges and descriptions - customize for your content types",
      "additionalInstructions": "Extra guidance for the AI evaluator"
    }
  };
  
  await Deno.writeTextFile(criteriaPath, JSON.stringify(exampleConfig, null, 2));
...
function validateCriteriaConfig(config: RankingCriteriaConfig): void {
  ...
  if (!config.criteria || !Array.isArray(config.criteria) || config.criteria.length === 0) {
    throw new Error("Criteria config must have at least one criterion");
  }
  
  if (!config.scoringGuidelines || !Array.isArray(config.scoringGuidelines) || config.scoringGuidelines.length === 0) {
  ...
  for (const criterion of config.criteria) {
    if (!criterion.id || !criterion.name || !criterion.description) {
      throw new Error(`Invalid criterion: missing required fields (id, name, description)`);
    }
  }
  ...
  for (const guideline of config.scoringGuidelines) {
    if (!guideline.range || !guideline.description) {
      throw new Error(`Invalid scoring guideline: missing required fields (range, description)`);
    }
  }
}
 ...
 */
export function generateCriteriaPromptText(criteria: RankingCriteriaConfig): string {
  ...
  if (criteria.additionalInstructions && criteria.additionalInstructions.length > 0) {
